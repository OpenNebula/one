...
std::vector<std::string> QuotaVirtualMachine::VM_METRICS = {"VMS", "RUNNING_VMS",
                                                            "CPU", "RUNNING_CPU",
                                ...
                                string& error)
{
    ...
    for (const string& metric : VM_METRICS)
    {
        if (metric == "SYSTEM_DISK_SIZE")
        {
            continue;
        }
        ...
        {
            vm_request.insert(make_pair(metric, value));
        }
    }
...
void QuotaVirtualMachine::del(Template * tmpl)
{
    ...
    auto size = VirtualMachineDisks::system_ds_size(tmpl, true);
                                 ...
                                 string& error)
{
    ...
    auto delta_size = VirtualMachineDisks::system_ds_size(tmpl, true);
