...
class KVMDomain
    ...
    def initialize(vm_xml, vm_dir, opts = {})
        ...
        @ds_id = File.basename(File.dirname(vm_dir))

        @backup_id = 0
        @parent_id = -1

        case @vm.elements['BACKUPS/BACKUP_CONFIG/MODE']&.text
        when 'FULL'
            @backup_id = 0
            @parent_id = -1
        ...
        interactive = @vm.elements['BACKUPS/BACKUP_CONFIG/INTERACTIVE']
    ...
    def backup_nbd_live_interactive(disks_s)
        init    = Time.now
        disks   = disks_s.split ':'
        exports = {}
        ...
        @vm.elements.each 'TEMPLATE/DISK' do |d|
            ...
            exports[did] = {
                :source   => mkuri(tgt),
                :target   => "#{@bck_dir}/disk.#{did}.#{@backup_id}",
                :exporter => 'nbd',
                :format   => 'qcow2',
                :map      => map,
                :size     => d.elements['SIZE'].text.to_i
            }
        end

        log("[BCK]: Incremental backup ready for export in #{Time.now - init}s")
    ...
    def backup_full_live_interactive(disks_s)
        init  = Time.now
        disks = disks_s.split ':'
        dspec = []
        qdisk = {}
        ...
        qdisk.each do |did, disk|
            Nbd.start_nbd(disk.path)
            sources << disk.path

            exports[did] = {
                :source   => Nbd.uri(disk.path),
                :target   => "#{@bck_dir}/disk.#{did}.0",
                :exporter => 'nbd',
                :format   => 'qcow2',
                :size     => disk_info[did][:size]
            }
        end

        log("[BCK]: Full live backup ready for export in #{Time.now - init}s")
        ...
        sources.each {|source| Nbd.stop_nbd(source) } if sources
    end
    ...
    def backup_nbd_interactive(disks_s)
        init    = Time.now
        disks   = disks_s.split ':'
        dids    = []
        sources = []
        ...
        dids.each do |d|
            source = "#{@vm_dir}/disk.#{d}"

            Nbd.start_nbd(source, nbd_map)

            sources << source

            exports[d] = {
                :source   => Nbd.uri(source),
                :target   => "#{@bck_dir}/disk.#{d}.#{@backup_id}",
                :exporter => 'nbd',
                :format   => 'qcow2',
                :map      => map,
                :size     => disk_info[d][:size]
            }
        end

        log("[BCK]: CBT incremental ready for export in #{Time.now - init}s")
    ...
    def backup_full_interactive(disks_s)
        init    = Time.now
        disks   = disks_s.split ':'
        exports = {}
        sources = []
        ...
        @vm.elements.each 'TEMPLATE/DISK' do |d|
            ...
            Nbd.start_nbd(sdisk.path)
            sources << sdisk.path

            exports[did] = {
                :source => Nbd.uri(sdisk.path),
                :target => "#{@bck_dir}/disk.#{did}.0",
                :exporter => 'nbd',
                :format => 'qcow2',
                :size => d.elements['SIZE'].text.to_i
        ...
        log("[BCK]: Full backup ready for export in #{Time.now - init}s")
