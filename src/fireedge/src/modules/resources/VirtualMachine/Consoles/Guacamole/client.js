...
const { CONNECTING, DISCONNECTING, DISCONNECTED, CLIENT_ERROR, TUNNEL_ERROR } =
  GUACAMOLE_STATES_STR
 ...
 * @param {boolean} options.isReady - Whether the client can start connection
...
const GuacamoleClient = ({
  ...
  useEffect(() => {
    ...
    guac.current.tunnel.oninstruction = (opcode, args) => {
      ...
      if (opcode === 'sync' && !hasNotifiedConnection.current) {
        hasNotifiedConnection.current = true
        enqueueSuccess(T.SuccessConnectionEstablished)
      }
    }
    ...
    guac.current.client.onstatechange = (state) => {
      ...
      if (isDisconnected && hasNotifiedConnection.current) {
        hasNotifiedConnection.current = false
        enqueueInfo(T.InfoDisconnected)
      }
  ...
  useEffect(() => {
    isReady && !session.isConnected && handleConnect()
  }, [token, isReady])
