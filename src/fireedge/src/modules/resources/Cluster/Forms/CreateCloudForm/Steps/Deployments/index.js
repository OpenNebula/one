...
import { getStyles } from '@modules/resources/Cluster/Forms/CreateCloudForm/Steps/Deployments/styles'
      ...
      <Grid container spacing={2} sx={styles.container}>
        {selectedDriver?.deploymentConfs?.map((conf) => (
          <Grid
            item
            xs={6}
            key={conf.deploymentName}
            data-cy={`${conf.deploymentInventory}`}
          ...
          >
            ...
            >
              <Stack direction="column" sx={styles.cardContent}>
                <Typography sx={styles.title}>{conf.deploymentName}</Typography>
