...
import {
  ...
  PUNCTUAL_FIELDS,
  RELATIVE_FIELDS,
} from '@modules/resources/resources/VirtualMachine/Forms/CreateSchedActionForm/fields'
...
const COMMON_SCHEMA = object({
  ...
  ARGS: object().when(ACTION_FIELD_NAME, (action) => {
    ...
    return (getRequiredArgsByAction(actionName) ?? []).reduce(
      (result, arg) => result.concat(object({ [arg]: ARG_SCHEMAS[arg] })),
      object()
    )
  }),
})
