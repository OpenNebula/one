...
const isNicPciDevice = (device) => String(device?.CLASS ?? '').startsWith('02')

const getAssignablePciDevices = (hosts = []) =>
  hosts
    .map(getPciDevices)
    .flat()
    .filter((device) => !isNicPciDevice(device))
...
const NAME_FIELD = {
  ...
  values: () => {
    ...
    const pciDevices = getAssignablePciDevices(hosts)
...
const PROFILE_FIELD = {
  ...
  values: (dependencies = []) => {
    ...
    if ((deviceVendorClass || shortAddress) && data) {
      const pciDevices = getAssignablePciDevices(data)
 ...
 * @param {object} params.oneConfig - Config of oned.conf
 * @param {boolean} params.adminGroup - User is admin or not
