...
import AlertText from '@modules/resources/resources/VirtualMachine/Forms/CreateSchedActionForm/AlertText'
import {
  Field,
  arrayToOptions,
  isDate,
...
const getTomorrowAtMidnight = () =>
  getTomorrow().set({ hour: 12, minute: 0, second: 0 }).toJSDate()

const getNextWeek = () =>
  getNow().plus({ weeks: 1 }).set({ hour: 12, minute: 0, second: 0 })

const getSafePeriodicityByTimeInSeconds = (seconds) => {
  try {
    return getPeriodicityByTimeInSeconds(seconds)
  } catch {
    return undefined
  }
}

const isValidDate = (value) => isDate(value) && !Number.isNaN(value.getTime())

const formatScheduleActionDays = (days) => {
  const values = getScheduleActionDays(days)

  return values.length ? values.join(',') : undefined
}

const parseDateString = (value, originalValue) => {
  if ([undefined, null, ''].includes(originalValue)) return originalValue
  if (isValidDate(originalValue)) return originalValue
  if (originalValue?.isValid) return originalValue.toJSDate()

  if (isRelative(originalValue)) {
    const newValueInSeconds =
      getSafePeriodicityByTimeInSeconds(originalValue)?.time
  ...
  if (Number.isFinite(numberValue)) {
    const valueInSeconds =
      numberValue > 9999999999 ? Math.trunc(numberValue / 1000) : numberValue

    return timeFromMilliseconds(valueInSeconds).toJSDate()
  }
...
const TIME_FIELD = {
  ...
  validation: date()
    ...
    .when(PERIODIC_FIELD_NAME, (typeAction, schema) =>
      typeAction !== SCHEDULE_TYPE.RELATIVE ? schema.required() : schema.strip()
    ),
  fieldProps: {
    minDateTime: getNow(),
  },
  ...
  validation: array(string())
    ...
    .when(REPEAT_FIELD.name, (repeatType, schema) =>
      repeatType !== REPEAT_VALUES.WEEKLY
        ? schema.strip()
        : schema.min(1).required(T.DaysBetween0_6)
    ),
  grid: { md: 12 },
}
...
const MONTHLY_FIELD = {
  ...
  validation: string()
    ...
    .when(REPEAT_FIELD.name, (repeatType, schema) =>
      repeatType !== REPEAT_VALUES.MONTHLY ? schema.strip() : schema.required()
    ),
}
...
const DAYS_FIELD = {
  ...
  validation: mixed()
    ...
    .transform((value, _originalValue, context) => {
      ...
      return formatScheduleActionDays(dayValues[repeatType] ?? value)
    }),
}
