...
import { getStyles } from '@modules/resources/resources/VnTemplate/Tabs/Info/styles'
...
export const Info = ({ data, config }) => {
  ...
  const selectedVnTemplates = useMemo(
    () => (selected ? [selected].flat() : [vnTemplate].filter(Boolean)),
    [selected, vnTemplate]
  )
  const isAggregated = selectedVnTemplates.length > 1

  const aggregatedPermissions = useMemo(
    () => aggregatePermissions(selectedVnTemplates),
    [selectedVnTemplates]
  )

  const aggregatedOwnership = useMemo(
    () => aggregateOwnership(selectedVnTemplates),
    [selectedVnTemplates]
  )
