...
import {
  ...
  removeNode,
  stripTreeMetadata,
} from '@modules/resources/resources/Settings/NestedLabelTree/utils'
import { GroupAPI, UserAPI, useGeneralApi, oneApi } from '@FeaturesModule'
import { T } from '@ConstantsModule'
import { isDevelopment, encodeLabels, jsonToXml } from '@UtilsModule'
      ...
      if (userTree) {
        promises.push(
          update(updateUser, {
            id: uId,
            template: jsonToXml(
              { FIREEDGE: { LABELS: encodeLabels(userTree) } },
              { addRoot: true, encode: false }
            ),
            replace: 1,
          })
