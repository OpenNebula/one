...
const ConfigurationSection = ({ stepId, oneConfig, adminGroup, isUpdate }) => {
  ...
  const handleAddUserKey = useCallback(
    (event) => {
      ...
      if (currentKey?.includes(SSH_KEY_USER)) return
  ...
  const sshPublicKey = useWatch({ control, name: SSH_PUBLIC_KEY_PATH })
  const isDisabled = sshPublicKey?.includes(SSH_KEY_USER) ?? false
