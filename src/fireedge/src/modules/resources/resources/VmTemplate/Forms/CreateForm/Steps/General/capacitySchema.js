...
} from '@modules/resources/resources/VmTemplate/Forms/CreateForm/Steps/General/capacityUtils'
import {
  Field,
  arrayToOptions,
  prettyBytes,
...
const HelperDiskCost = () => {
  ...
  const numericCost = isNaN(cost) || cost == null ? 0 : +cost / 1024
  const monthlyCost = formatNumberByCurrency(numericCost * 24 * 30)

  return sprintf(T.CostEachMonth, monthlyCost)
}
...
export const MEMORY_COST = generateCostCapacityInput({
  ...
  fieldProps: ([memory, cost] = []) => {
    ...
    const numericCost = isNaN(cost) || cost == null ? 0 : +cost / 1024
    const numericMemory = isNaN(memory) || memory == null ? 0 : +memory
    const monthlyCost = formatNumberByCurrency(
      numericMemory * numericCost * 24 * 30
    )
    fieldProps.hint = sprintf(T.CostEachMonth, monthlyCost)
...
export const CPU_COST = generateCostCapacityInput({
  ...
  fieldProps: ([cpu, cost] = []) => {
    ...
    const numericCost = isNaN(cost) || cost == null ? 0 : +cost / 1024
    const numericCpu = isNaN(cpu) || cpu == null ? 0 : +cpu
    const monthlyCost = formatNumberByCurrency(
      numericCpu * numericCost * 24 * 30
    )
