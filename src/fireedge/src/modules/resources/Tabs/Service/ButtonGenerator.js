...
export const ButtonGenerator = ({ items, options = {} }) => {
  ...
  if (Array.isArray(items)) {
    return (
      <Box>
        {options?.button?.buttonType === 'iconOnly' ? (
          <SubmitButton
            aria-controls="customized-menu"
            aria-haspopup="true"
            onClick={handleClick}
            {...options?.button}
            iconOnly={options?.button?.iconOnly ?? <NavArrowDown />}
          />
        ) : (
          <SubmitButton
            aria-controls="customized-menu"
    ...
    return options?.singleButton?.buttonType === 'iconOnly' ? (
      <SubmitButton
        aria-controls="customized-menu"
        aria-haspopup="true"
        ...
        iconOnly={options?.singleButton?.iconOnly ?? <NavArrowDown />}
      />
    ) : (
      <SubmitButton
        onClick={(event) => handleClick(event, items.onClick)}
        startIcon={items.iconOnly || null}
        {...options?.singleButton}
        sx={{
          ...options?.singleButton?.sx,
        }}
