...
import {
  SCHEMA,
  FIELDS,
} from '@modules/componentsv2/composed/Forms/ChangeGroupForm/schema'

const ChangeGroupFormConfig = createForm(SCHEMA, FIELDS)

/**
 * Renders the change group form config inside a modal form host.
 *
...
export const ChangeGroupForm = ({
  ...
  const { transformBeforeSubmit, ...formConfig } = useMemo(
    () => ChangeGroupFormConfig(stepProps, initialValues),
...
/**
 * Returns a modal-ready ChangeGroupForm component.
 *
 * @param {object} configProps - Form configuration props
 * @returns {Function} Wrapped form component
 */
export const getChangeGroupForm = (configProps = {}) => {
  /**
   * @param {object} rootProps - Modal form props
   * @returns {*} Wrapped change group form
   */
  const WrappedChangeGroupForm = (rootProps) => (
    <ChangeGroupForm {...configProps} {...rootProps} />
  )

  WrappedChangeGroupForm.displayName = 'ChangeGroupForm'

  return WrappedChangeGroupForm
}

export { SCHEMA, FIELDS }

export default ChangeGroupFormConfig
