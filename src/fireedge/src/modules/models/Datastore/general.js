 ...
 */
export const getDatastoreType = ({ TYPE } = {}) => {
 ...
 */
export const getDatastoreState = ({ STATE = 0 } = {}) => DATASTORE_STATES[STATE]
...
/**
 * Aggregates datastore enabled state across multiple datastores.
 *
 * @param {object[]} datastores - Datastores with a STATE field
 * @returns {{ allEnabled: boolean, allDisabled: boolean }} - Aggregate enabled status
 */
export const aggregateDatastoreEnabledState = (datastores) =>
  [].concat(datastores).reduce(
    (acc, datastore) => {
      const isEnabled = datastore?.STATE === '0'

      return {
        allEnabled: acc.allEnabled && isEnabled,
        allDisabled: acc.allDisabled && !isEnabled,
      }
    },
