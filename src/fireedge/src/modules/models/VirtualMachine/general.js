...
import { getLocked, stringToBoolean } from '@UtilsModule'
...
/**
 * @param {VM} vm - Virtual machine
 * @returns {HistoryRecord|undefined} Active or last history record
 */
export const getActiveHistory = (vm) => {
  const history = [].concat(vm?.HISTORY_RECORDS?.HISTORY)

  return history.find(
    (h) => h?.ETIME == null || h?.ETIME === '0' || h?.ETIME === 0
  )
}
...
/**
 * @param {VM} vm - Virtual machine
 * @returns {string|undefined} VM hostname from active history record
 */
export const getVmHostname = (vm) => getActiveHistory(vm)?.HOSTNAME
 ...
 * @returns {Snapshot[]} List of snapshots from resource
 */
export const getBackupList = (vm) => {
  const { BACKUPS = {} } = vm ?? {}

  return [BACKUPS?.BACKUP_IDS?.ID].filter(Boolean).flat()
}
...
export const getScheduleActions = (vm) => {
  ...
  return [TEMPLATE.SCHED_ACTION]
    ...
    .map((action) => {
      ...
      const ensuredWarn = isRelativeWarning ? ensuredTime + +WARNING : +WARNING
...
/**
 * Check if action is available for **all VMs** based on state.
 *
 * @param {object} action - VM action
 * @param {VM|VM[]} vms - Virtual machines
 * @returns {boolean} If `true`, the action is available for all VMs
 */
export const getVmActionAvailableStates = (action, vms = []) => {
  const vmList = [vms].flat()
  const hypervisors = [
    ...new Set((vmList.length ? vmList : [{}]).map((vm) => getHypervisor(vm))),
  ]
  const actionStates = hypervisors.map(
    (hypervisor) => VM_ACTIONS_BY_STATE[hypervisor]?.[action]
  )

  if (actionStates.some((states) => !states)) return []

  const restrictedStates = actionStates.filter((states) => states.length)

  if (!restrictedStates.length) return []

  return restrictedStates.reduce((commonStates, states) =>
    commonStates.filter((state) => states.includes(state))
  )
}
