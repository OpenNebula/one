 ...
 */
export const getHostState = (host) => HOST_STATES[+host?.STATE ?? 0]
...
export const getAllocatedInfo = (host) => {
  ...
  const percentCpuUsed = +MAX_CPU > 0 ? (+CPU_USAGE * 100) / +MAX_CPU || 0 : 0
  const percentCpuLabel = `${CPU_USAGE} / ${+MAX_CPU > 0 ? MAX_CPU : '-'} (${
    +MAX_CPU > 0 ? Math.round(percentCpuUsed) : '--'
  }%)`

  const isMemUsageNegative = +MEM_USAGE < 0
  const percentMemUsed = +MAX_MEM > 0 ? (+MEM_USAGE * 100) / +MAX_MEM || 0 : 0
  const usedMemBytes = prettyBytes(Math.abs(+MEM_USAGE))
  const totalMemBytes = prettyBytes(+MAX_MEM)
  const percentMemLabel = `${isMemUsageNegative ? '-' : ''} ${usedMemBytes} / ${
    +MAX_MEM > 0 ? totalMemBytes : '-'
  }  (${+MAX_MEM > 0 ? Math.round(percentMemUsed) : '--'}%)`
 ...
 */
export const getHostHypervisors = (options = {}, data = {}) => {
