...
import { parseAcl } from '@UtilsModule'

import { ACL_TYPE_ID, ACL_RIGHTS, T, ACL_USERS } from '@ConstantsModule'
...
export const translateACL = (rule, users, groups, clusters, zones) => {
  ...
  // Define the message
  let message = T['acls.translate.rule'] + ' '
  ...
  if (
    ...
    message += T['acls.translate.user.id'] + ' '
    if (acl.USER.id) {
      message += acl.USER.id + ' '
    }
    if (acl.USER.name) {
    ...
    message += T['acls.translate.user.group'] + ' ' + acl.USER.id + ' '
    if (acl.USER.name) {
      message += ' (' + acl.USER.name + ') '
    }
  } else if (acl.USER?.type && acl.USER.type === ACL_USERS.ALL.type) {
    message += T['acls.translate.user.all'] + ' '
  }

  // Rights info
  message += T['acls.translate.rights'] + ' '
  ...
  rights?.forEach((right, index) => {
    if (index === rights.length - 1) {
      ...
      message += right + ' ' + T['acls.translate.and'] + ' '
    }
  })

  if (rights?.length > 1) {
    message += ' ' + T['acls.translate.operations'] + ' '
  } else {
    message += ' ' + T['acls.translate.operation'] + ' '
  }
  ...
  if (
    ...
    message += T['acls.translate.overall'] + ' '
  } else {
    message += T['acls.translate.over'] + ' '
  }
  ...
  acl.RESOURCE?.resources.forEach((resource, index) => {
    if (index === acl.RESOURCE.resources.length - 1) {
      ...
      message += resource + ' ' + T['acls.translate.and'] + ' '
    }
  })
  ...
  if (
    ...
    message +=
      ' ' + T['acls.translate.resource.id'] + ' ' + acl.RESOURCE.identifier.id
  } else if (
    acl.RESOURCE?.identifier.type &&
    ...
    message +=
      ' ' +
      T['acls.translate.resource.group'] +
      ' ' +
      acl.RESOURCE.identifier.id
    ...
    message +=
      ' ' +
      T['acls.translate.resource.cluster'] +
      ' ' +
      acl.RESOURCE.identifier.id
    if (acl.RESOURCE.identifier.name) {
      message += ' (' + acl.RESOURCE.identifier.name + ')'
  ...
  if (acl.ZONE?.type && acl.ZONE?.type === ACL_USERS.INDIVIDUAL.type) {
    message += ' ' + T['acls.translate.zone.id'] + ' ' + acl.ZONE.id
    if (acl.ZONE.name) {
      message += ' (' + acl.ZONE.name + ')'
    }
  } else if (acl.ZONE?.type === ACL_USERS.ALL.type) {
    message += ' ' + T['acls.translate.zone.all']
  }
