...
# Udev rules file containing manual remappings, generated by one-deploy (helper/pci role). Actual
# values (PCI slot, interface name) are extracted from a specially formatted comment.
UDEV_RENAMES = extract_pci_interfaces('/etc/udev/rules.d/99-rename.rules')
...
def get_interface_name(device)
    ...
    # Store in cache
    begin
        Dir.mkdir(NET_NAMES_CACHE_DIR) unless Dir.exist?(NET_NAMES_CACHE_DIR)
        File.write(File.join(NET_NAMES_CACHE_DIR, pci_addr), "#{iface_name}\n")
    rescue StandardError
