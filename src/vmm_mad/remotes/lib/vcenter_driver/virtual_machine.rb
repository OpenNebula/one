...
module VCenterDriver
    ...
    class VirtualMachine < VCenterDriver::Template
        ...
        def wild?
            ...
            one_item['TEMPLATE/IMPORTED'] == 'YES')
        end
            ...
            if vi_client.rp_confined?
                ...
                if req_rp_ref
                    rps =
                        cluster
                        .resource_pools
                        .select {|r| r._ref == req_rp_ref }

                    if rps.empty?; raise "No matching resource pool found (#{req_rp})."; end
        ...
        def get_ds
            ...
            current_ds     = VCenterDriver::VIHelper
                             .one_item(
        ...
        def customization_get
            ...
            begin
                ...
                raise 'Error getting customization spec'; end
        ...
        # @return VCenterDriver::Datastore datastore where
        # the disk will live under
        ...
        # This function creates a new VM from the driver_action XML
        # and returns the VMware ref
        # @param drv_action XML representing the deploy action
        # @return String vmware ref
        def clone_vm(drv_action)
            vcenter_name = vcenter_name_get
            ...
            if ds.instance_of? RbVmomi::VIM::Datastore
                use_linked_clones =
                    drv_action['USER_TEMPLATE/VCENTER_LINKED_CLONES']
            ...
            if !vcenter_vm_folder.nil? && !vcenter_vm_folder.empty?
                vcenter_vm_folder_object =
                    dc
                    .item.find_folder(vcenter_vm_folder)
            end

            if vcenter_vm_folder_object.nil?
                vcenter_vm_folder_object = vc_template.parent
            end
            ...
            if ds.instance_of? RbVmomi::VIM::StoragePod
                ...
                begin
                    ...
                    vm = storagepod_clonevm_task(clone_parameters)
        ...
        # TODO: review
        def storagepod_clonevm_task(clone_parameters)
            vc_template = clone_parameters[:vc_template]
            vcenter_name = clone_parameters[:vcenter_name]
            clone_spec = clone_parameters[:clone_spec]
            storpod = clone_parameters[:storpod]
            vcenter_vm_folder_object = 
            clone_parameters[:vcenter_vm_folder_object]
            dc = clone_parameters[:dc]
            ...
            result_res_to_rec = !result.respond_to?(:recommendations)

            if result_res_to_rec || result.recommendations.empty?
                raise 'Could not get placement specification for StoragePod'
            end
            ...
            begin
                ...
                if vm
                    ...
                    begin
                        ...
                        key = result.recommendations.first.key ||= ''
                        if key.empty?
                            raise 'Missing Datastore \
                            recommendation for StoragePod'
                        end
                        ...
                        raise "Failure applying recommendation \
                        while cloning VM: #{e.message}"
                    end
                end
            end
        end
        ...
        #
        # @param one_disk [XMLelement]
        # The OpenNebula object representation of the disk
        # @param keys [Hash (String => String)] Hashmap
        # with the unmanaged keys
        # @param vc_disks [Array (vcenter_disks)]
        ...
        def disk(index, opts = {})
            ...
            opts[:keys].nil? ? keys = disk_keys_get : keys = opts[:keys]
            if opts[:disks].nil?
                vc_disks = vcenter_disks_get
            else
                vc_disks = opts[:disks]
        ...
        def reference_unmanaged_devices(template_ref, execute = true)
            ...
            xpath =
                'TEMPLATE/DISK[OPENNEBULA_MANAGED="NO" or \
            ...
            begin
                        ...
                        nics.each do |nic|
                            ...
                            if type == NET_CARD
                                ...
                                opaque_network = opaque_networks.find do |opn|
                                    ...
                                    nic_opaque_id == opn_opaque_id
                                end
                                nref = opaque_network._ref
                            else
                                raise "Unsupported network card \
                                type: #{nic.backing.class}"
                            end
                ...
                make sure that they are working properly. Error: #{e.message}"
            end
        ...
        def reference_all_disks
            ...
            # keys for disks are prefixed
            # with opennebula.disk and opennebula.mdisk
            @item.config.extraConfig.each do |elem|
                if elem.key.start_with?('opennebula.disk.')
                    disks_extraconfig_current[elem.key] =
                        elem
                        .value
                end
                next unless elem.key.start_with?('opennebula.mdisk.')
            ...
            num_disks_difference =
                disks_extraconfig_current
                .keys
                .count - disks_vcenter_current.count
            ...
            disks_vcenter_current.each do |item|
                ...
                if disks_has_key && disks_has_equal
                    update = true
                end
                # check if vCenter disk hasn't got a representation
                # in the extraConfig
            ...
            # Avoids having an old disk in the configuration that
            ...
            disks_extraconfig_new.keys.each do |key|
                extraconfig_new << {
                    :key => key,
                        :value => disks_extraconfig_new[key]
                }
            end
        ...
        def reference_disks(template_ref, disks, managed)
            ...
            disks.each do |disk|
                ...
                if template_disk
                    vcenter_disk =
                        vcenter_disks
                        .select {|d| d[:key] == template_disk[:key] }.first
                end
        ...
        def create_storagedrs_disks(device_change_spod, device_change_spod_ids)
            ...
            device_change_spod.each do |device_spec|
                disk_locator << RbVmomi::VIM
                                .PodDiskLocator(
                                    :diskId => device_spec[:device].key
                                )
            end
            ...
            device_change_spod.each do |device_spec|
                unit_number =
                    device_spec[:device].unitNumber
                controller_key =
                    device_spec[:device].controllerKey
                key =
                    get_vcenter_disk_key(unit_number, controller_key)
                disk_id =
                    device_change_spod_ids["#{controller_key}-#{unit_number}"]
                reference = {}
                reference[:key] =
                    "opennebula.disk.#{disk_id}"
                reference[:value] =
                    key.to_s
        ...
        def boot_order(boot_info)
            ...
            boot_order_value =
                boot_info
                .split(',').map {|str| convert.call(str) }

            RbVmomi::VIM
                .VirtualMachineBootOptions(
                    {
                        :bootOrder => boot_order_value
                    }
                )
        end
        ...
        #
        # @param option  [symbol]  if :all is provided the method
        # will try to sync
        # all the nics (detached and not existing ones) otherwise
        # it will only sync
        # the nics that are not existing
        #
        # @param execute [boolean] indicates if the reconfigure
        ...
        def sync_nics(option = :none, execute = true)
            ...
            if option == :all
                ...
                # detached? condition indicates that the
                # nic exists in OpeNebula but not
                # in vCenter
        ...
        def sync(deploy = {})
            ...
            if deploy[:boot] && !deploy[:boot].empty?
                boot_opts = boot_order(deploy[:boot])
            end
        ...
        def regenerate_context
            ...
            begin
                ...
                raise "Cannot create snapshot for VM: \
                #{e.message}\n#{e.backtrace}"
            end
        end
        ...
        def calculate_add_nic_spec(nic, unumber = nil)
            ...
            default =
                ...
                .get_default('VM/TEMPLATE/NIC/MODEL')
            ...
            limit_in  =
                nic['INBOUND_PEAK_BW'] || VCenterDriver::VIHelper
                .get_default('VM/TEMPLATE/NIC/INBOUND_PEAK_BW')
            limit_out =
                nic['OUTBOUND_PEAK_BW'] || VCenterDriver::VIHelper
                .get_default('VM/TEMPLATE/NIC/OUTBOUND_PEAK_BW')
            limit     = nil
            ...
            rsrv_in =
                nic['INBOUND_AVG_BW'] || VCenterDriver::VIHelper
                .get_default('VM/TEMPLATE/NIC/INBOUND_AVG_BW')
            rsrv_out =
                nic['OUTBOUND_AVG_BW'] || VCenterDriver::VIHelper
                .get_default('VM/TEMPLATE/NIC/OUTBOUND_AVG_BW')
            rsrv = nil
            ...
            if !unumber
                @unic =
                    @unic || vcenter_nics_get
                    .map {|d| d.unitNumber }.max || 0
                unumber = @unic += 1
            else
                @unic   = unumber
            end
        ...
        def detach_nic(mac)
            ...
            begin
                ...
                raise "Cannot detach NIC from VM: \
                #{e.message}\n#{e.backtrace.join("\n")}"
            end
        end
        ...
        def detach_all_nics
            ...
            begin
                ...
                raise "Cannot detach all NICs from VM: \
                #{e.message}\n#{e.backtrace}"
            end
        end
        ...
        def sync_extraconfig_disk(spec_hash)
            ...
            spec_hash[:deviceChange].each_with_index do |_device, index|
                ...
                vc_disks.each do |vc_disk|
                    ...
                    extraconfig_new <<
                        {
                            :key => spec_hash[:extraConfig][index][:key],
        ...
        # @param option  [symbol]  if :all is provided the method
        # will try to sync
        # all the disks (detached and not existing ones) otherwishe
        # it will only sync
        # the disks that are not existing
        #
        # @param execute [boolean] indicates if
        # the reconfigure operation is going to
        # be executed
        def sync_disks(option = :nil, execute = true)
            ...
            if option == :all
                detach_op = {}
                detach_op[:deviceChange],
                detach_op[:extraConfig] = detach_disks_specs

                device_change_empty !detach_op[:deviceChange].empty?
                extra_config__empty = !detach_op[:extraConfig].empty?
                perform = device_change_empty || extra_config__empty
        ...
        def attach_disk(disk)
            ...
            if disk_attached_to_vm(disk, unmanaged_keys, vc_disks)
                raise 'DISK is already connected to VM'
            end
            ...
            begin
                if storpod
                    ...
                    # Disk id is -1 as I don't know what
                    # disk id is going to be set
                    ...
                    # Add the key for the volatile disk to
                    # the unmanaged opennebula.disk.id variables
                    unit_number =
                ...
                raise "Cannot attach DISK to VM: \
                #{e.message}\n#{e.backtrace.join("\n")}"
            end
        end
        ...
        def detach_persistent_disks(vm)
            ...
            begin
                ...
                raise "Cannot detach all DISKs from VM: \
                #{e.message}\n#{e.backtrace}"
            end
        end
        ...
        def detach_disk(disk)
            ...
            begin
                ...
                raise "Cannot detach DISK from VM: \
                #{e.message}\n#{e.backtrace}"\
        ...
        def convert_to_template
            ...
            one_nics.each do |nic|
                ...
                nic_specs <<
                    calculate_add_nic_spec_autogenerate_mac(nic)
            end
        ...
        def instantiated_as_persistent?
            begin
                ...
                # one_item may not be retrievedif
                # deploy_id hasn't been set
                false
            end
        end
        ...
        def find_free_ide_controller(_position = 0)
            ...
            devices.each do |dev|
                v_disk = (dev.is_a? RbVmomi::VIM::VirtualDisk)
                v_cdrom = (dev.is_a? RbVmomi::VIM::VirtualCdrom)
                next unless v_disk || v_cdrom

                if ide_schema.key?(dev.controllerKey)
                    ide_schema[dev.controllerKey][:freeUnitNumber]
                        .delete(dev.unitNumber)
                end
            end
        ...
        def find_free_controller(position = 0)
            ...
            15.times do |scsi_id|
                next unless used_numbers
                            .grep(scsi_id).length <= 0
        ...
        def add_new_scsi(scsi_schema, devices)
            ...
            devices.each do |device|
                ...
                next unless vir_log && device.key == scsi_key
            ...
            if vcenter_version != '5.5'
                ...
                until snapshot_created || elapsed_minutes == 1440
                    if !@item['snapshot'].nil?
                        current_snapshot =
                            @item['snapshot.currentSnapshot'] rescue nil
                        snapshot_found =
                            find_snapshot_in_list(
                                @item['snapshot.rootSnapshotList'],
                                snap_id
                            )
                        snapshot_created =
                            !snapshot_found.nil? &&
                            !current_snapshot.nil? &&
                            current_snapshot._ref == snapshot_found._ref
                    end
                    sleep(60)
                    elapsed_minutes += 1
                end
        ...
        def revert_snapshot(snap_id)
            ...
            begin
                ...
                raise "Cannot revert snapshot of VM: \
                #{e.message}\n#{e.backtrace.join("\n")}"
            end
        end
        ...
        def delete_snapshot(snap_id)
            ...
            begin
                ...
                raise "Cannot delete snapshot of VM: \
                #{e.message}\n#{e.backtrace.join("\n")}"
            end
        end
        ...
        def migrate(config = {})
            if config.empty?
                raise 'You need at least 1 parameter to \
                perform a migration'
            end
        ...
        def remove_all_snapshots(consolidate = true)
            @item
                .RemoveAllSnapshots_Task({ :consolidate => consolidate })
                .wait_for_completion
            info_disks
        end
        ...
        def self.migrate_routine(
            ...
            if ds
                ...
                ds_pool.info
                datastore =
                    ds_pool["/DATASTORE_POOL/\
                    DATASTORE[ID='#{ds}']/TEMPLATE/VCENTER_DS_REF"]
            end
        ...
        # @param opts [Hash] object with pairs that
        # could contain multiple option
        ...
        def self.new_from_clone(vi_client, drv_action, id)
            spawn = new(vi_client, nil, id).tap do |vm|
