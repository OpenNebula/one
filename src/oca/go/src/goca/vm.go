...
// VMPool returns a new vm pool controller.
func (c *Controller) VMPool() *VMsController {
...
func (c *Controller) VMByName(name string) (uint, error) {
	var id uint

	vmPool, err := (&VMsController{c}).Info()
	if err != nil {
		return 0, err
	}
...
func (vc *VMsController) Monitoring(filter int) error {
	_, err := vc.c.Client.Call("one.vmpool.monitoring", filter)
	return err
}
...
func (vc *VMController) Create(template string, pending bool) (uint, error) {
...
func (vc *VMController) Monitoring() error {
	_, err := vc.c.Client.Call("one.vm.monitoring", vc.ID)
	return err
}
...
func (vc *VMController) DiskSaveas(diskID int, imageName, imageType string, snapID int) error {
	_, err := vc.c.Client.Call("one.vm.disksaveas", vc.ID, diskID, imageName, imageType, snapID)
	return err
}

// DiskSnapshotCreate will create a snapshot of the disk image
func (vc *VMController) DiskSnapshotCreate(diskID int, description string) error {
	_, err := vc.c.Client.Call("one.vm.disksnapshotcreate", vc.ID, diskID, description)
	return err
}

// DiskSnapshotDelete will delete a snapshot
func (vc *VMController) DiskSnapshotDelete(diskID, snapID int) error {
	_, err := vc.c.Client.Call("one.vm.disksnapshotdelete", vc.ID, diskID, snapID)
	return err
}

// DiskSnapshotRevert will revert disk state to a previously taken snapshot
func (vc *VMController) DiskSnapshotRevert(diskID, snapID int) error {
	_, err := vc.c.Client.Call("one.vm.disksnapshotrevert", vc.ID, diskID, snapID)
	return err
}
...
func (vc *VMController) DiskSnapshotRename(diskID, snapID int, newName string) error {
	_, err := vc.c.Client.Call("one.vm.disksnapshotrename", vc.ID, diskID, snapID, newName)
	return err
}
...
// XML
func (vc *VMController) Attach(diskTemplate string) error {
	_, err := vc.c.Client.Call("one.vm.attach", vc.ID, diskTemplate)
	return err
}

// Detach a disk from a virtual machine
func (vc *VMController) Detach(diskID int) error {
	_, err := vc.c.Client.Call("one.vm.detach", vc.ID, diskID)
	return err
}

// DiskResize a disk of a virtual machine
func (vc *VMController) DiskResize(diskID int, size string) error {
	_, err := vc.c.Client.Call("one.vm.diskresize", vc.ID, diskID, size)
	return err
}
...
func (vc *VMController) DetachNic(nicID string) error {
