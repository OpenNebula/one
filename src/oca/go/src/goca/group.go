...
// GroupByName returns a Group ID from name
func (c *Controller) GroupByName(name string) (uint, error) {
	var id uint

	groupPool, err := (&GroupsController{c}).Info()
	if err != nil {
		return 0, err
	}
...
// GroupPool returns a group pool. A connection to OpenNebula is
// performed.
...
func (gc *GroupController) Create(name string) (uint, error) {
