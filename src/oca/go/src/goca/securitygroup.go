...
// SecurityController is a controller for Security entities
...
func (c *Controller) SecurityGroupByName(name string) (uint, error) {
	var id uint

	secgroupPool, err := (&SecurityGroupsController{c}).Info()
	if err != nil {
		return 0, err
	}
...
// SecurityGroupPool returns a security group pool. A connection to OpenNebula is
// performed.
...
func (sc *SecurityGroupController) Create(tpl string) (uint, error) {
...
func (sc *SecurityGroupController) Update(tpl string, appendTemplate int) error {
	_, err := sc.c.Client.Call("one.secgroup.update", sc.ID, tpl, appendTemplate)
	return err
}
