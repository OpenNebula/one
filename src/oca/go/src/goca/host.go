...
// HostController is a controller for create an host or a pool of hosts
...
// HostByName finds a Host ID from name
func (c *Controller) HostByName(name string) (uint, error) {
	var id uint

	hostPool, err := (&HostsController{c}).Info()
	if err != nil {
		return 0, err
	}
...
func (hc *HostController) Monitoring() error {
	_, err := hc.c.Client.Call("one.host.monitoring", hc.ID)
	return err
}
