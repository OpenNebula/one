...
func (c *Controller) VMGroup(id uint) *VMGroupController {
	return &VMGroupController{c, id}
}

// VMGroupByName returns a VMGroup ID from name
func (c *Controller) VMGroupByName(name string, args ...int) (uint, error) {
	var id uint

	vmGroupPool, err := (&VMGroupsController{c}).Info(args...)
	if err != nil {
		return 0, err
	}
...
func (vc *VMGroupController) Create(tpl string) (uint, error) {
