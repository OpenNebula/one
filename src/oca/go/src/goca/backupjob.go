...
func (bjc *BackupJobController) Update(tpl string) error {
	return bjc.UpdateContext(context.Background(), tpl)
}

// UpdateContext adds Backup Job content.
// * tpl: The new Backup Job content. Syntax can be the usual attribute=value or XML.
func (bjc *BackupJobController) UpdateContext(ctx context.Context, tpl string) error {
	_, err := bjc.c.Client.CallContext(ctx, "one.backupjob.update", bjc.ID, tpl)
	return err
}
