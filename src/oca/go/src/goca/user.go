...
// UserByName returns a User by Name
func (c *Controller) UserByName(name string) (uint, error) {
	var id uint

	userPool, err := (&UsersController{c}).Info()
	if err != nil {
		return 0, err
	}
...
// UserPool returns a user pool. A connection to OpenNebula is
// performed.
...
func (uc *UserController) Create(name, password, authDriver string, groupIDs []uint) (uint, error) {
