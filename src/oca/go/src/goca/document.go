...
// DocumentsController is a controller for VMs
type DocumentsController entitiesController

// DocumentController is a controller for VM entity
...
func (c *Controller) Documents() *DocumentsController {
	return &DocumentsController{c}
}
...
func (c *Controller) DocumentByName(name string, documentType int) (uint, error) {
	var id uint

	documentPool, err := (&DocumentsController{c}).Info(documentType)
	if err != nil {
		return 0, err
	}
...
func (dc *DocumentController) Create(tpl string, documentType int) (uint, error) {
	response, err := dc.c.Client.Call("one.document.allocate", tpl, documentType)
	if err != nil {
		return 0, err
	}
