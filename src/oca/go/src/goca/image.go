...
// Imagess returns an Images controller
...
func (c *Controller) ImageByName(name string) (uint, error) {
	var id uint

	imagePool, err := (&ImagesController{c}).Info()
	if err != nil {
		return 0, err
	}
...
func (ic *ImageController) SnapshotDelete(snapID int) error {
	_, err := ic.c.Client.Call("one.image.snapshotdelete", ic.ID, snapID)
	return err
}

// SnapshotRevert reverts image state to a previous snapshot
func (ic *ImageController) SnapshotRevert(snapID int) error {
	_, err := ic.c.Client.Call("one.image.snapshotrevert", ic.ID, snapID)
	return err
}

// SnapshotFlatten flattens the snapshot image and discards others
func (ic *ImageController) SnapshotFlatten(snapID int) error {
	_, err := ic.c.Client.Call("one.image.snapshotflatten", ic.ID, snapID)
	return err
}
