...
func (c *Controller) TemplateByName(name string) (uint, error) {
	var id uint

	templatePool, err := (&TemplatesController{c}).Info()
	if err != nil {
		return 0, err
	}
...
// TemplatePool returns a template pool. A connection to OpenNebula is
// performed.
...
// change
func (tc *TemplateController) Chmod(uu, um, ua, gu, gm, ga, ou, om, oa int) error {
	_, err := tc.c.Client.Call("one.template.chmod", tc.ID, uu, um, ua, gu, gm, ga, ou, om, oa)
	return err
}
...
func (tc *TemplateController) Instantiate(name string, pending bool, extra string) (uint, error) {
	response, err := tc.c.Client.Call("one.template.instantiate", tc.ID, name, pending, extra)
