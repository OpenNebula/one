...
// MarketPlaceAppByName returns a MarketPlace ID from name
func (c *Controller) MarketPlaceAppByName(name string) (uint, error) {
	var id uint

	marketAppPool, err := (&MarketPlaceAppsController{c}).Info()
	if err != nil {
		return 0, err
	}
...
// MarketPlaceAppPool returns a marketplace app pool. A connection to OpenNebula is
// performed.
