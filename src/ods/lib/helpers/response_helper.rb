...
module OpenNebula
    ...
    module DocumentServer
        ...
        module ResponseHelper
            ...
            # Handles a generic exception and passes it to `internal_error`.
            # Includes backtrace only in debug mode.
            # @param error [Exception] The exception to handle.
            def general_error(error)
                context = Log.debug? ? { 'backtrace' => error.backtrace.join("\n") } : {}
                internal_error(error.message, GENERAL_EC, context)
            end
            ...
            def internal_error(error, error_code, context = {})
                ...
                error_context = { 'details' => error_context.to_s } unless error_context.is_a?(Hash)

                # Log the error in the log file
                log_msg  = "Error code #{error_code}: #{error_msg}"
                log_msg += ", context: #{error_context}" if Log.debug? && !context.empty?

                Log.error COMP, log_msg
        ...
        # Custom ValidationError for Document Schemas
        class ValidationError < OpenNebula::Error

            attr_reader :context

            # @param message [String] error description
            # @param context [Hash] validation context (schema errors, etc.)
            # @param errno [Integer] OpenNebula error code
            def initialize(message, context:, errno: OpenNebula::Error::EINTERNAL)
                @context = context
                super(message, errno)
            end
