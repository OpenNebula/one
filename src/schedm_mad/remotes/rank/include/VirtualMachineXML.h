...
public:
    ...
    VirtualMachineXML(const xmlNodePtr node, xmlNodePtr requirement_xml): ObjectXML(node)
    {
        init_attributes(requirement_xml);
    }
    ...
    const std::vector<int>& get_prereq_host_ids() const {return prerequied_host_ids;};

    const std::vector<int>& get_prereq_ds_ids() const {return prerequied_ds_ids;};

    const std::map<int, std::vector<int>>& get_prereq_nics() const {return prerequied_nics;};
    ...
    bool test_image_datastore_capacity(
            std::shared_ptr<ImageDatastorePoolXML> img_dspool, std::string & error_msg) const;

    /**
     *  Adds the VM disk requirements to each Image Datastore counter
     *    @param img_datastores Image Datastores
     */
    void add_image_datastore_capacity(std::shared_ptr<ImageDatastorePoolXML> img_dspool);
...
protected:
     ...
     *  For constructors
     */
    void init_attributes(xmlNodePtr requirement_xml = nullptr);
    ...
    std::vector<int> prerequied_host_ids = {};
    std::vector<int> prerequied_ds_ids = {};

    std::map<int, std::vector<int>> prerequied_nics = {};
