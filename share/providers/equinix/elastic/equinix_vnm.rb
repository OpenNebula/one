...
class EquinixProvider < GenericProvider

    def initialize(provider, host)
        super(provider, host)
        @eq = Equinix.new(@connection[:auth_token])
    end
    ...
    def assign(_ip, external, _opts = {})
        ...
        unless ['201', '422'].include?(resp.code)
            STDERR.puts "Error assigning #{external}: #{resp.code} - #{resp.message}"
            return 1
        end
    ...
    def unassign(_ip, external, _opts = {})
        ...
        resp = @eq.api_call("/devices/#{@resource_id}/ips")
