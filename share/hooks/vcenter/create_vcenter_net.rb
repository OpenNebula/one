...
def logicalSwitchData(nsxmgr, virtualWireID)
    header = {'Content-Type': 'application/xml'}
    uri = URI.parse("https://#{nsxmgr}/api/2.0/vdn/virtualwires/#{virtualWireID}")
...
def createLogicalSwitch(nsxmgr, pgType, vdnscopeID, virtualWire)
    ...
    if pgType == "NSX-V"
        ...
        File.open('/tmp/r.debug', 'w') { |file| file.write(r.inspect) }
...
begin
    ...
    if one_vnet["VN_MAD"] == "vcenter" && managed && imported.nil?
        ...
        if pg_type == "NSX-V"
            ...
            ls_new = createLogicalSwitch(nsxmgr, pg_type, vdnscope_id, virtualWireSpec)
            # Update one network with portgroup name generated by nsx-v
            # vnet_ref,pg_name = logicalSwitch(nsxmgr, lsNew)
            ls_vni = logicalSwitchData(nsxmgr, ls_new)
            update_net(one_vnet,"BRIDGE=\"#{ls_new}\"\nNSX_VNI=\"#{ls_vni}\"")
        end
        ...
        if pg_type == "Opaque Network"
            ...
            ls_new = createLogicalSwitch(nsxmgr, pg_type, vdnscope_id, virtualWireSpec.to_s)
            update_net(one_vnet,"BRIDGE=\"#{ls_new}\"\nNSX_VNI=\"pending\"")
        end
